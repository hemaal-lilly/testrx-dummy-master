...
When('the page renders', async function (this: ICustomWorld) {
  // No additional actions required, page is already loaded.
});
...
When('the hero section is viewed', async function (this: ICustomWorld) {
  // No additional actions required, hero section is already visible.
});
...
When('the cards under the "Automation Tech for Tech" section are viewed', async function (this: ICustomWorld) {
  // No additional actions required, cards section is already visible.
});
...
When('each displayed card under the "Automation Tech for Tech" section is inspected', async function (this: ICustomWorld) {
  // No additional actions required, cards are already visible.
});
...
When('the browser window is resized between common mobile and desktop widths', async function (this: ICustomWorld) {
  await pageObject.resizeBrowser(375, 667); // Example mobile size
  await pageObject.resizeBrowser(1280, 800); // Example desktop size
});
...
Then('the "Automation Tech for Tech" section remains visible', async function (this: ICustomWorld) {
